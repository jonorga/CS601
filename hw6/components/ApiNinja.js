const _API_KEY = "QGdAEDhrVVL3WV+ijtl6rQ==LV7EqaPsJG3HyiFU";
...
export function getDadJoke(targetId) {
    fetch(_BASE_URL, {
        ...
        return new Promise(resolve => {
            const jokeCharLen = joke.length;
            resolve({joke, jokeCharLen});
        });
    }).then(result => {
        const charCountLi = document.createElement("li");
        charCountLi.innerHTML = "Joke character count: " + result.jokeCharLen;
        document.querySelector("#metrics").appendChild(charCountLi);
        return new Promise(resolve => {
            const jokeWordLen = result.joke.split(" ").length;
            resolve({jokeWordLen});
        });
    }).then(wordCount => {
        const wordCountLi = document.createElement("li");
        wordCountLi.innerHTML = "Joke word count: " + wordCount.jokeWordLen;
        document.querySelector("#metrics").appendChild(wordCountLi);
    }).catch(error => {
